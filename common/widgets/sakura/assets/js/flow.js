...
(function (window, document, undefined) {
     ...
     * @param {Array.<string>} [opts.accept]
    ...
    function Flow(opts) {
        ...
        this.onDrop = function (event) {
            ...
            if ($.opts.singleFile && event.dataTransfer.files.length > 1) {
                $.fire('fileError', null, "You dragged more than one photo! Please only drag your desired profile photo.");
                return;
            }

            if (!$.acceptValidate(event.dataTransfer.files)) {
        ...
        this.opts.accept = $.acceptFilters(this.opts.accept);
    ...
    Flow.prototype = {
            ...
            for (var i = 0, len = accept.length; i < len; i++) {
                ...
                if (mime.length > 1) {
                    if (mime[1] === '*') {
                        ...
                        for (var j = 0, jlen = this.fileTypes.length; j < jlen; j++) {
                            fileType = this.fileTypes[j].split(/\//);
                            if (mime[0] === fileType[0]) mimes.push(this.fileTypes[j]);
                        }
                    } else {
                    ...
                    if (mime[0] in this.filters) mimes.push(this.filters[mime[0]]);
                }
            }
            return mimes;
        },
        /**
         * Returns true or false if file type is allowed to upload or not.
         * @function
         * @param files
         * @returns {bool}
         */
        acceptValidate: function (files) {
            ...
            for (var i = 0, len = files.length; i < len; i++) {
                ...
                for (var j = 0, jlen = this.opts.accept.length; j < jlen; j++) {
                    accepted = (files[i].type === this.opts.accept[j]);
                    if (accepted) break;
                }
                if (accepted) break;
            }
         ...
         */
        acceptMimetypes: function (mimetypes) {

            var mimes = mimetypes.split(/,/), exts = [];
            for (var i = 0, len = mimes.length; i < len; i += 2) {
                this.fileTypes.push(mimes[i]);
                exts = mimes[i + 1].split(/ /);
                for (var j = 0, jlen = exts.length; j < jlen; j++) {
                    this.filters[exts[j]] = mimes[i];
                }
            }
        },
    };
